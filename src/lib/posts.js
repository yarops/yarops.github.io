...
export async function getSidePosts() {
  ...
  return {
    sidePosts: Array.isArray(posts) && posts.map(mapPostData),
  };
}
...
export async function getPaginatedPostsByCategoryId({ categoryId, currentPage = 1, ...options } = {}) {
  const { queryIncludes = 'index' } = options;
  const { posts } = await getPostsByCategoryId({categoryId, queryIncludes: queryIncludes});
